...
function plusReady() {
	...
	lotName = wv.cls
	lotText = wv.class_name
	console.log(lotName,lotText)
	$("#title").text(lotText)
	switch(lotName){
...
//          }
  console.log(JSON.stringify(data))
           if(data.code == 1){
               ...
               $('#bet-play').attr('data-id',data[0]['play_id'])//2409
...
/*function getPlayListMainSubMenu(name,id,seri){
	...
	$.ajax({
        ...
        dataType:'json',
        data: {'lottery':lotName,'play_id':id,'sign':sign,'user_id':user_id,'token':token,play_two_id:'0'},
        type:'get',
        success:function(data){
...
function goBet(){
 ...
 //盘口
        $("#playMenu li").click(function () {
            liindex = $(this).index();
            console.log(liindex)
            $(this).addClass("active").siblings("li").removeClass("active");
            $(".mod-cons").eq(liindex).show().siblings(".mod-cons").hide();
            $(".clearinput").click();
            if (liindex >= 3 && liindex <= 5) {
                $("#orderType li").eq(0).click();
                $("#orderType li").slice(0, 3).hide();
                $(".QuickAmount,.QuickAmounts").show(200);
            } else if ($("#orderType li.on").index() != 1) {
                $(".QuickAmount,.QuickAmounts").hide(200);
                $("#orderType li").slice(0, 3).show();
            } else {
                $("#orderType li").slice(0, 3).show();
            }
            return false;

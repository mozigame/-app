...
function plusReady() {
	...
	console.log(wv)
	lot = wv.lot_id;
	var lot_name =wv.lot_name;
	console.log(wv.lot_id)
	console.log(wv.lot_name)
	$('.sz_logo>strong').html(lot_name);
...
$(".szmenu a").click(function() {
	...
	fullsztable(OpenListnewArray)
});

function fullsztable(result) {
...
function show_user_pl(){
	var json = {
		'user_id': user_id,
		'token': token,
		'sign': sign
	};
	var lot = window.localStorage['lot_id'];
	var json1 = {
		'user_id': user_id,
		'token': token,
		'sign': sign,
	...
	mui.ajax(uri + 'Account/GetUserInfo', {
		...
		type: 'get',
		success: function(data){
		var fdz = data.flevel_fd;
		window.localStorage['fdz'] = fdz;
		}
	})
	mui.ajax(uri + 'Common/GetLotteryAll', {
		...
		success: function(data){
			...
			var pl = [];
			$.each(data.game, function(i,v) {
				var fdz = window.localStorage['fdz'];
				var num = parseFloat(v.odds)+(parseFloat(v.odds)*parseFloat(fdz)*0.01111);
			...
			$('#xfdz').text(pl[3]);
			$('#xdfdz').text(pl[4]);
//		var zdpl = data.game.10.odds;
//		var num = zdpl+zdpl*fdz*0.01111;
//		var total = num.toFixed(3);
//		$('.zhuangdui>strong').html(total);
//		
//		var zpl = data.game.11.odds;
//		var num = zpl+zpl*fdz*0.01111;
//		var total = num.toFixed(3);
//		$('.long>strong').html(total);
//		
//		var hpl = data.game.12.odds;
//		var num = hpl+hpl*fdz*0.01111;
//		var total = num.toFixed(3);
//		$('.hu>strong').html(total);
//		
//		var xpl = data.game.13.odds;
//		var num = xpl+xpl*fdz*0.01111;
//		var total = num.toFixed(3);
//		$('.he>strong').html(total);
//		
//		var xdpl = data.game.14.odds;
//		var num = xdpl+xdpl*fdz*0.01111;
//		var total = num.toFixed(3);
//		$('.xiandui>strong').html(total);
		}
	})
	...
	mui.ajax(uri + 'Common/GetNoLotteryInfo', {
		...
		success: function(data) {
			...
			if(data.number == '' || typeof(data.number) == 'undefined') {
				...
				$('.tit_1>strong').html(arr[0]);
				$('.tit_2>strong').html(arr[1]);
				$('.tit_3>strong').html(arr[3]);
				$('.tit_4>strong').html(arr[4]);
...
function get_room_record(search_time_s, search_time_e, record_no, order_status, lottery, page_no) {
	...
	console.log(JSON.stringify(json))
	mui.ajax(uri + 'Report/GetBetRecord', {
		data: json,
		dataType: 'json',
		type: 'get',
		success: function(data) {
